...
const deliveryRoutes = async (fastify) => {
    const deliveryService = new delivery_service_1.DeliveryService(fastify);
    ...
    fastify.get('/active', {
        ...
        try {
            ...
            const groupedOrders = {
                accepted: activeOrders.filter(o => o.status === client_1.OrderStatus.ACCEPTED),
                preparing: activeOrders.filter(o => o.status === client_1.OrderStatus.PREPARING),
                onTheWay: activeOrders.filter(o => o.status === client_1.OrderStatus.ON_THE_WAY)
            };
            return reply.send({
                success: true,
