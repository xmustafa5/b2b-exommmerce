...
const vendorRoutes = async (fastify) => {
    const vendorService = new vendor_service_1.VendorService(fastify);
    // Middleware to check if user is a vendor or company manager
    const requireVendorAccess = [
        auth_1.authenticate,
        (0, auth_1.requireRole)(client_1.UserRole.COMPANY_ADMIN, client_1.UserRole.COMPANY_USER, client_1.UserRole.LOCATION_ADMIN, client_1.UserRole.SUPER_ADMIN)
    ];
    ...
    fastify.put('/products/:id', {
        ...
        try {
            ...
            const product = await vendorService.updateProduct(request.params.id, company.id, request.body);
            return reply.send({
                success: true,
                product,
    ...
    fastify.patch('/products/:id/stock', {
        ...
        try {
            ...
            const product = await vendorService.updateProductStock(request.params.id, company.id, quantity, operation);
            return reply.send({
                success: true,
                product,
    ...
    fastify.get('/orders', {
        ...
        try {
            ...
            if (request.user.role === client_1.UserRole.SUPER_ADMIN || request.user.role === client_1.UserRole.LOCATION_ADMIN) {
    ...
    fastify.patch('/orders/:id/status', {
        ...
        try {
            ...
            const order = await vendorService.updateOrderStatus(request.params.id, company.id, status, comment);
            return reply.send({
                success: true,
                order,
    ...
    fastify.get('/export/:type', {
        ...
        try {
            ...
            const data = await vendorService.exportVendorData(company.id, request.params.type, format);
    ...
    fastify.get('/orders/:id', {
        ...
        try {
            ...
            const vendorDiscount = order.items.reduce((sum, item) => sum + (item.discount || 0), 0);
