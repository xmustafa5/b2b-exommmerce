...
const payoutRoutes = async (fastify) => {
    const payoutService = new payout_service_1.PayoutService(fastify);
    ...
    fastify.post('/request', {
        ...
        try {
            ...
            if (![client_1.UserRole.COMPANY_ADMIN, client_1.UserRole.COMPANY_ADMIN].includes(user.role)) {
    ...
    fastify.get('/balance', {
        ...
        try {
            const user = request.user;
            if (![client_1.UserRole.COMPANY_ADMIN, client_1.UserRole.COMPANY_ADMIN, client_1.UserRole.LOCATION_ADMIN, client_1.UserRole.SUPER_ADMIN].includes(user.role)) {
                return reply.code(403).send({
                    error: 'Forbidden',
    ...
    fastify.patch('/:id/status', {
        ...
        try {
            ...
            const payout = await payoutService.updatePayoutStatus(id, status, request.user.id, notes);
            return reply.send({
                success: true,
                payout,
    ...
    fastify.post('/report', {
        ...
        try {
            ...
            const report = await payoutService.generatePayoutReport(targetCompanyId, new Date(startDate), new Date(endDate));
            return reply.send({
                success: true,
                report
            });
    ...
    fastify.post('/schedule', {
        ...
        try {
            ...
            if (![client_1.UserRole.COMPANY_ADMIN, client_1.UserRole.LOCATION_ADMIN, client_1.UserRole.SUPER_ADMIN].includes(user.role)) {
                return reply.code(403).send({
                    error: 'Forbidden',
