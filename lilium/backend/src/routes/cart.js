...
const cartRoutes = async (fastify) => {
    const cartService = new cart_service_1.CartService(fastify);
    ...
    fastify.post('/promotions', {
        ...
        try {
            ...
            // Apply promotions
            const itemsWithPromotions = await cartService.applyPromotions(validatedItems, request.user.userId);
            // Get updated summary
            const summary = await cartService.getCartSummary(itemsWithPromotions.map(item => ({
