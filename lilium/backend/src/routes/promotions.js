...
const promotionRoutes = async (fastify) => {
    const promotionService = new promotion_service_1.PromotionService(fastify);
    // Get all promotions
    fastify.get('/', {
        schema: {
            tags: ['promotions'],
    ...
    fastify.post('/', {
        ...
        try {
            const data = validation_1.createPromotionSchema.parse(request.body);
    ...
    fastify.put('/:id', {
        ...
        try {
            const { id } = request.params;
            const data = validation_1.updatePromotionSchema.parse(request.body);
    ...
    fastify.post('/apply-to-cart', {
        ...
        try {
            const data = validation_1.applyPromotionSchema.parse(request.body);
            const result = await promotionService.applyPromotionsToCart(data.cartItems);
            return reply.send(result);
        }
        catch (error) {
            return (0, errors_1.handleError)(error, reply, fastify.log);
