...
const analyticsRoutes = async (fastify) => {
    const analyticsService = new analytics_service_1.AnalyticsService(fastify);
    // ============================================
    ...
    fastify.get('/dashboard/vendor', {
        ...
        try {
            ...
            if (![client_1.UserRole.COMPANY_ADMIN, client_1.UserRole.COMPANY_USER, client_1.UserRole.SUPER_ADMIN, client_1.UserRole.LOCATION_ADMIN].includes(user.role)) {
    ...
    fastify.get('/dashboard/admin', {
        preHandler: [auth_1.authenticate, (0, auth_1.authorize)([client_1.UserRole.SUPER_ADMIN, client_1.UserRole.LOCATION_ADMIN])],
        schema: {
            tags: ['analytics'],
    ...
    fastify.post('/reports/sales', {
        ...
        try {
            ...
            if (user.role === client_1.UserRole.COMPANY_ADMIN || user.role === client_1.UserRole.COMPANY_USER) {
                targetCompanyId = user.companyId;
            }
            else if (![client_1.UserRole.SUPER_ADMIN, client_1.UserRole.LOCATION_ADMIN].includes(user.role)) {
                return reply.code(403).send({
                    error: 'Forbidden',
            ...
            const report = await analyticsService.generateSalesReport(targetCompanyId, { startDate: new Date(startDate), endDate: new Date(endDate) });
    ...
    fastify.post('/reports/commission', {
        ...
        try {
            ...
            const report = await analyticsService.generateCommissionReport(targetCompanyId, { startDate: new Date(startDate), endDate: new Date(endDate) });
            return reply.send({ success: true, report });
        }
        catch (error) {
            return (0, errors_1.handleError)(error, reply, fastify.log);
