...
const orderRoutes = async (fastify) => {
    const orderService = new order_service_1.OrderService(fastify);
    ...
    fastify.get('/', {
        ...
        try {
            const query = validation_1.orderQuerySchema.parse(request.query);
            ...
            const result = await orderService.getOrders(query.page, query.limit, filters, user.role, user.zones);
    ...
    fastify.get('/stats', {
        ...
        try {
            ...
            let statsZone = zone;
            if (user.role === client_1.UserRole.LOCATION_ADMIN && user.zones && user.zones.length > 0) {
    ...
    fastify.post('/', {
        preHandler: [auth_1.authenticate, (0, auth_1.requireRole)(client_1.UserRole.SHOP_OWNER)],
        schema: {
            tags: ['orders'],
        ...
        try {
            const user = request.user;
            const data = validation_1.createOrderSchema.parse(request.body);
            const orderData = {
                ...data,
                userId: user.userId,
            };
    ...
    fastify.put('/:id/status', {
        ...
        try {
            const { id } = request.params;
            const { status, note } = validation_1.updateOrderStatusSchema.parse(request.body);
            const user = request.user;
