...
class CategoryService {
    ...
    async deleteCategory(id, reassignToId) {
        ...
        if (category._count.products > 0 || category._count.children > 0) {
            if (!reassignToId) {
                throw this.fastify.httpErrors.conflict('Category has products or subcategories. Provide reassignToId to move them.');
            }
    ...
    async reorderCategories(orders) {
        const updates = orders.map(({ id, sortOrder }) => this.fastify.prisma.category.update({
            where: { id },
            data: { sortOrder },
        }));
