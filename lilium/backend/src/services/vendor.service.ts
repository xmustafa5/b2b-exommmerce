...
import { Company, Product, Order, OrderItem, User, Zone, OrderStatus, Prisma } from '@prisma/client';
...
export class VendorService {
   ...
   */
  async getVendorCompany(vendorUserId: string): Promise<Company | null> {
  ...
  async updateOrderStatus(
    ...
    const validTransitions: Record<OrderStatus, OrderStatus[]> = {
      ...
      [OrderStatus.FAILED]: [OrderStatus.PROCESSING], // Can retry
    };
   ...
   */
  async getVendorStats(companyId: string): Promise<VendorDashboardStats> {
    ...
    const processingOrders = orders.filter(o =>
      [OrderStatus.ACCEPTED, OrderStatus.PROCESSING, OrderStatus.READY].includes(o.status)
    ).length;
    ...
    const totalRevenue = orders
      ...
      .reduce((sum, order) => {
        const orderRevenue = order.items.reduce(
          (itemSum, item) => itemSum + (item.price * item.quantity - (item.discount || 0)),
          0
        );
